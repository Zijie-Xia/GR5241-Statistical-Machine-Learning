...
```{r}
R=1000
y.test.mat = matrix(nrow = 2, ncol = R)
mat.pred.1 = matrix(nrow = 5, ncol = R)
mat.pred.2 = matrix(nrow = 5, ncol = R)
for (i in 1:R){
  sim.data.train = sim.training(x.test)
  y.test.mat[,i] = t(sim.data.train$y.test)
  prediction = poly.predict(degree.vec=1:5,
                            data=sim.data.train$data.train,
                            x.test=x.test)
  mat.pred.1[,i] = t(prediction[1,])
  mat.pred.2[,i] = t(prediction[2,])
}
```
## Task 2
...
```{r}
mse.10 = mean((y.test.mat[1,]-mat.pred.1[1,])^2)
mse.16 = mean((y.test.mat[2,]-mat.pred.2[1,])^2)
var.10 = var(mat.pred.1[1,])
var.16 = var(mat.pred.2[1,])
f = true.f(c(10,16))
square.bias.10 = (mean(mat.pred.1[1,])-f[1])^2
square.bias.16 = (mean(mat.pred.2[1,])-f[2])^2
for (i in 2:5){
  mse.10 = c(mse.10,mean((y.test.mat[1,]-mat.pred.1[i,])^2))
  mse.16 = c(mse.16,mean((y.test.mat[2,]-mat.pred.2[i,])^2))
  var.10 = c(var.10,var(mat.pred.1[i,]))
  var.16 = c(var.16,var(mat.pred.2[i,]))
  square.bias.10 = c(square.bias.10,(mean(mat.pred.1[i,])-f[1])^2)
  square.bias.16 = c(square.bias.16,(mean(mat.pred.2[i,])-f[2])^2)
}
mse.10 
var.10
square.bias.10
...
```{r}
library(tidyverse)
value.10 = data.frame(degree=1:5,mse.10,var.10,square.bias.10)%>%
  pivot_longer(cols = 2:4)

g1 = ggplot(data = value.10,aes(x=degree,y=value,group=name))+
...
```
```{r}
value.16 = data.frame(degree=1:5,mse.16,var.16,square.bias.16)%>%
  pivot_longer(cols = 2:4)

g2 = ggplot(data = value.16,aes(x=degree,y=value,group=name))+
  geom_line(aes(color=name))+
  geom_point(aes(color=name))+
  labs(
    title = 'When x = 16',
